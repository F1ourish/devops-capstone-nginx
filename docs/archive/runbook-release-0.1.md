# Runbook (Archive: Release 0.1)

Этот документ сохранён как архивная версия раннего этапа проекта.

Он относится к стадии, где архитектура проекта отличалась от текущей Compose-first модели, поэтому не должен использоваться как основной operational runbook для актуальной версии репозитория.

Актуальная версия runbook находится в docs/runbook.md.
